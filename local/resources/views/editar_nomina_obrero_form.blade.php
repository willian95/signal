...
@section('content')
    ...
    <div id="modal_nuevo_empleado" class="modal modal-fixed-footer" style="height: 800px;">
      <div class="modal-content">
        <h4>Ingresar obrero</h4>
        <form method="post" action="{{url('/ingresar_personal_obrero_flotante/'.$id_nomina)}}" id="flotante_form">
    ...
    ?>
          <tr>
            ...
            <td>
            	<a class="waves-effect btn" href="{{url('/editar_personal_obrero_nomina/'.$nomina->id_empleado.'/nomina/'.$id_nomina)}}"><i class="material-icons">edit</i></a>
    ...
    function verificar_flotante(){
      var usuario = document.getElementById('empleado_id').value;
      if(usuario){
        document.getElementById('flotante_form').submit();
      }
      else{
        Materialize.toast('Seleccione un empleado', 4000);
      }
    }
