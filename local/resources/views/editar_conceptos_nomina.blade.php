...
@section('content')
    ...
    @if(session('alert'))
      ...
      @endif
<div>
           <form method="post" action="{{url('/insertar_concepto_nomina/'.$id_nomina.'/empleado/'.$id_empleado)}}">
          ...
          <p class="center-align"><a href="{{url('/editar_nomina_obrero_form/'.$id_nomina)}}" class="waves-effect waves-light btn red"><i class="material-icons left">reply</i>atras</a></p>
    ...
    <table class="bordered">
		...
		@foreach($conceptos as $concepto)
          <tr>
            ...
            <td>
                <form method="post" action="{{url('/editar_concepto/'.$concepto->concepto_id.'/id/'.$concepto->id)}}" id="form_editar{{$concepto->id}}">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
            ...
            <td>
              @if($concepto->concepto_id == 5440 || $concepto->concepto_id == 5450 || $concepto->concepto_id == 5460 ||  $concepto->concepto_id == 5471 || $concepto->concepto_id == 5472 || $concepto->concepto_id == 30111 || $concepto->concepto_id == 30121 || $concepto->concepto_id == 20111 || $concepto->concepto_id == 20121 || $concepto->concepto_id == 20222 || $concepto->concepto_id == 20134 || $concepto->concepto_id == 20131 || $concepto->concepto_id == 20136 || $concepto->concepto_id == 30131 || $concepto->concepto_id == 30172)
               ...
               <a class="waves-effect btn" onclick="editar({{$concepto->id}})"><i class="material-icons">edit</i>
                  <form method="post" style="display:none;" action="{{url('/eliminar_personal_concepto_nomina/'.$concepto->id.'/nomina/'.$id_nomina)}}" id="form_editar{{$concepto->id}}">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  </form>
                </a>
            ...
            @endif
                ...
                <a class="waves-effect btn" style="background-color: #e53935;" onclick="eliminar({{$concepto->id}})"><i class="material-icons">delete</i>
                  <form method="post" style="display:none;" action="{{url('/eliminar_personal_concepto_nomina/'.$concepto->id.'/nomina/'.$id_nomina)}}" id="form_eliminar{{$concepto->id}}">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  </form>
                </a>
            </td>
